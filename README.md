...
4. **সফল** — সবুজ চেকমার্ক + Transaction ID। `successUrl` দেওয়া থাকলে ~২.৫ সেকেন্ড পর কাস্টমারকে সেই URL-এ রিডাইরেক্ট করে দেয় (সাথে `?status=verified&invoiceId=...&reference=...&trxId=...` যোগ হয়ে)।
5. **মেয়াদোত্তীর্ণ** — লাল ঘড়ি আইকন দেখিয়ে ঠিক **১ সেকেন্ড পর অটোমেটিক** `cancelUrl`-এ (দেওয়া থাকলে) নাহলে হোমপেজে রিডাইরেক্ট করে দেয় (`public/js/invoice.js` এর `handleExpiry()` ফাংশন)।

**`successUrl` / `cancelUrl` (ঐচ্ছিক, `POST /api/invoices`-এ পাঠানো যায়):**
এগুলো `callbackUrl` (server-to-server webhook) থেকে সম্পূর্ণ আলাদা জিনিস — এরা কাস্টমারের **ব্রাউজারকে** কোথায় ফিরিয়ে নেবে সেটা ঠিক করে (UX-এর জন্য), যেখানে `callbackUrl` আপনার সার্ভারকে জানায় পেমেন্ট আসলেই সত্যি হয়েছে কিনা (নির্ভরযোগ্যতার জন্য)। দুটোই একসাথে ব্যবহার করাই সঠিক — কখনো শুধু URL প্যারামিটার দেখে অর্ডার "paid" মার্ক করবেন না, সেটা সবসময় webhook থেকেই যাচাই করুন। শুধু http(s) URL গ্রহণযোগ্য (`sanitizeRedirectUrl` এটা `src/utils.js`-এ যাচাই করে)।
...
**ইতিমধ্যে চলমান একটা ডাটাবেসে `successUrl`/`cancelUrl` সাপোর্ট যোগ করতে** (নতুন ইনস্টলে `schema.sql`-এই এটা আছে, লাগবে না):
```bash
npm run db:migrate:success-cancel-url:remote
```
